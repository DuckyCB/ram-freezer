...
func ValidateOutput() {
	...
	if err != nil {
		logs.Log.Error(fmt.Sprintf("No se pudo cargar el estado: %v\n", err))
		os.Exit(1)
	}

	filePath := usbFolder + config.OutputFolder + config.OutputFile
	logs.Log.Info(fmt.Sprintf("Validando el archivo %s...\n", filePath))

	// Verificar si el archivo existe
	fileInfo, err := os.Stat(filePath)
	if os.IsNotExist(err) {
		msg := fmt.Sprintf("El archivo %s no fue generado.\n", filePath)
		exit_code := 1
	...
	logs.Log.Info(fmt.Sprintf("Tamaño total de RAM: %.2f GB\n", totalRAMGB))
	...
	logs.Log.Info(fmt.Sprintf("Tamaño del archivo: %.2f GB\n", fileSizeGB))
	logs.Log.Info(fmt.Sprintf("Límite inferior: %.2f GB\n", lowerLimit))
	logs.Log.Info(fmt.Sprintf("Límite superior: %.2f GB\n", upperLimit))

	// Verificar si el archivo es demasiado pequeño
	if fileSizeGB < 1 {
		msg := fmt.Sprintf("El archivo %s es demasiado pequeño para ser válido. Tamaño: %.2f GB\n", filePath, fileSizeGB)
		exit_code := 1
		utils.WriteStateVal(stateFilePath, state, msg, exit_code)
		logs.Log.Error(msg)
		os.Exit(exit_code)
	...
	if fileSizeGB < lowerLimit {
		msg := fmt.Sprintf("El archivo %s es más pequeño de lo esperado. Tamaño: %.2f GB, RAM esperada: %.2f GB\n", filePath, fileSizeGB, totalRAMGB)
		exit_code := 0
		utils.WriteStateVal(stateFilePath, state, msg, exit_code)
		logs.Log.Warn(msg)
		os.Exit(exit_code)
	} else if fileSizeGB > upperLimit {
		msg := fmt.Sprintf("El archivo %s es más grande de lo esperado. Tamaño: %.2f GB, RAM esperada: %.2f GB\n", filePath, fileSizeGB, totalRAMGB)
		exit_code := 0
		utils.WriteStateVal(stateFilePath, state, msg, exit_code)
		logs.Log.Warn(msg)
		os.Exit(exit_code)
	...
	// Si todo está bien
	msg := fmt.Sprintf("El archivo %s es válido. Tamaño: %.2f GB, memoria RAM fisica: %.2f GB\n", filePath, fileSizeGB, totalRAMGB)
	exit_code := 0
	utils.WriteStateVal(stateFilePath, state, msg, exit_code)
	logs.Log.Info(msg)
